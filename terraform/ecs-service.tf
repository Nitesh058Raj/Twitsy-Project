resource "aws_ecs_service" "twitsy_service" {
  name            = "twitsy-service"
  cluster         = aws_ecs_cluster.twitsy_cluster.arn
...
resource "aws_security_group" "twitsy_security_group" {
  name_prefix = "twitsy-sg"
